...
2. **节间衔接**：每节以导言段开篇衔接前文、预告本节结构；禁止节间"硬切"
...
- 当前分支 `add-近场动力学基本理论框架`（非 `main`），已有提交（第1、2、6章相关）
