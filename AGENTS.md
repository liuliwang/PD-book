...
- **责任边界**：输出数学/物理内容时须声明"AI 辅助生成，需经专业审阅"；不得替代作者做物理假设或理论判断。实践格式：每节末尾加一行 `\emph{AI 辅助生成，需经专业审阅。}`（ch03、ch04、ch09 等完整章节均已采用，新章节沿用）
...
- **符号冲突**：写作前先读 notation.tex 已有符号，勿重名。已确认占用：`m`=加权体积（ch06）、`a`/`d`/`b_F` 等=复合材料参数（ch06）、`β`/`γ`=稳定化参数（ch07）、`c`=微模量（ch04/ch06）、`M`=单位方向态（ch06 下划线形式）。ch09 用 `n=δ/Δx`（网格比）避开 `m`，加速度用 `\ddot{\mathbf{u}}` 避开 `a`
...
- **禁止在 `.tex` 文件中使用 Markdown 语法**：包括 `**加粗**`、`*斜体*`、`# 标题`、`` `代码` `` 等。LaTeX 强调用 `\textbf{}`/`\emph{}`，代码用 `\texttt{}` 或 listings 环境
- **AI 自查**：每次写完/修改章节文件后，grep 检查 `\*\*`（两个连续星号）应零命中；若命中须立即改为 LaTeX 写法或删除强调
- **正文保持朴素**：书籍正文一般不用加粗强调关键词，重要概念用首次定义+术语表即可；确需强调时用 `\emph{}`
