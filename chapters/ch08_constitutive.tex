...
晶体塑性嵌入 NOSB 的路径与 8.1--8.3 节完全一致——本构对应机制（第 7 章\ref{sec:correspondence}节）：由非局部变形梯度构造经典应变度量，调用经典本构得到应力，再经力态公式构造键力。晶体塑性的特殊之处在于两点：其一，本构以晶格坐标系为参考系，需要把非局部变形梯度从全局坐标系旋转到晶格坐标系（取向映射）；其二，滑移率与硬化是路径相关的历史依赖本构，需要在每一材料点维护滑移系内变量（$g^{\alpha}$ 及累积滑移 $\gamma^{\alpha}$），并按 8.2 节的算子分裂格式推进 \cite{SunSundararaghavan2014,Littlewood2011}。
...
\end{equation}
式\eqref{eq:cp-cauchy-lattice}中 $\tilde{\boldsymbol{\sigma}}$ 为晶格系中的 Cauchy 应力，最后旋转回全局坐标系 $\boldsymbol{\sigma}=\mathbf{R}_{0}\tilde{\boldsymbol{\sigma}}\mathbf{R}_{0}^{\mathrm{T}}$（与第 7 章式\eqref{eq:rotate-back}的极分解—旋转流程一致 \cite{Warren2009}），经式\eqref{eq:hyper-force-state}构造力态。滑移率式\eqref{eq:cp-slip-rate-powerlaw}在晶格系中以分切应力 $\tilde{\tau}^{\alpha}=\tilde{\boldsymbol{\sigma}}:\mathbf{P}^{\alpha}_{\mathrm{sym}}$ 计算，其推导与式\eqref{eq:cp-resolved-shear}相同，仅应力张量换为晶格系值。

\begin{remark}
  式\eqref{eq:cp-lattice-deformation-gradient}的旋转方向需与全书符号一致核对：第 7 章未旋转构型方法（式\eqref{eq:unrotated-frame}）以极分解转动更新本构，本章晶体塑性以固定取向矩阵 $\mathbf{R}_{0}$ 把变形梯度旋转到晶格系，二者数学等价（都把本构放在随晶格转动的坐标系中执行），但实现路径不同：前者每步更新 $\mathbf{R}$，后者仅在初始化时固定 $\mathbf{R}_{0}$，晶格转动由 $\tilde{\mathbf{F}}^{\mathrm{e}}$ 的极分解隐式吸收 \cite{SunSundararaghavan2014}。
\end{remark}

多晶微结构的离散是第二步。多晶材料由取向各异的晶粒组成，每一晶粒赋予固定的取向矩阵 $\mathbf{R}_{0}$（由实验测得或随机生成），材料点按所在晶粒继承取向。在近场动力学离散中，材料点携带取向信息：点 $\mathbf{x}$ 的取向 $\mathbf{R}_{0}(\mathbf{x})$ 逐点赋值，晶界表现为相邻材料点取向的间断（图略，取向场 $\mathbf{R}_{0}(\mathbf{x})$ 的分片常数分布）。Sun 与 Sundararaghavan 2014 年首先把晶体塑性嵌入 NOSB，用隐式非线性求解器分析了平面多晶的弹塑性变形与剪切带演化 \cite{SunSundararaghavan2014}；Luo 等 2018 年引入自适应动力松弛（ADR）求解器处理准静态问题，捕捉了多晶中的微尺度剪切带 \cite{Luo2018}；Gu 等 2019 年加入罚函数稳定化并采用显式/隐式双求解器，实现了多晶的弹塑性变形与动态开裂的模拟，其中晶界处材料点的失效由第 11 章的损伤判据控制 \cite{GuZhangMadenci2019}。Littlewood 2011 年在 NOSB 中实现了晶体弹粘塑性模型，用于分析含硬质颗粒的铝合金单晶中的塑性变形与损伤累积，指出晶格取向对材料响应有强影响 \cite{Littlewood2011}。

数值实现的关键问题是 NOSB 的零能模式失稳（第 7 章\ref{sec:zero-energy}节）与滑移内变量的时间积分。零能模式源于非局部变形梯度式\eqref{eq:nonlocal-deformation-gradient}对近场域内高频变形模式的不敏感，在晶体塑性模拟中表现为变形场的数值振荡。文献中的处理包括罚函数稳定化（Gu 等 2019 \cite{GuZhangMadenci2019}）、应力点法（Luo--Sundararaghavan \cite{LuoSundararaghavan2018}）与高阶变形梯度近似（Littlewood 2011 \cite{Littlewood2011}），其原理在第 7 章统一讨论。滑移内变量的时间积分采用与 8.2--8.3 节一致的算子分裂格式：每一时间步内，（1）由当前构型计算非局部变形梯度式\eqref{eq:nonlocal-deformation-gradient}并经式\eqref{eq:cp-lattice-deformation-gradient}旋转到晶格系；（2）由式\eqref{eq:cp-plastic-velocity-gradient}计算塑性速度梯度，按式\eqref{eq:cp-slip-rate-powerlaw}求滑移率，用显式 Euler 格式更新 $\mathbf{F}^{\mathrm{p}}$ 与滑移阻力式\eqref{eq:cp-hardening-linear}；（3）由弹性部分式\eqref{eq:cp-f-decomp}计算 $\tilde{\mathbf{E}}^{\mathrm{e}}$ 与 $\tilde{\boldsymbol{\sigma}}$（式\eqref{eq:cp-cauchy-lattice}）；（4）旋转回全局系并经式\eqref{eq:hyper-force-state}构造力态，代入运动方程推进位移。该流程与 8.2--8.3 节的统一模式（点级内变量更新 + 经典本构调用 + 力态装配）完全一致，区别仅在于内变量由标量（记忆向量 $\mathbf{h}^{n}$、损伤 $D$）变为滑移系向量 $g^{\alpha}$ 与 $\mathbf{F}^{\mathrm{p}}$，体现了 NOSB 本构对应机制对"微结构内变量本构"的承载能力 \cite{SunSundararaghavan2014,GuZhangMadenci2019}。
