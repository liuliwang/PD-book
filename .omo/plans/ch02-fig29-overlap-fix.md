...
- [x] 1. 修复图 2.9 三处符号标记重叠：ξ+η 标签上移、η 标签下移、f 标签左移、引线改道（chapters/ch02_framework.tex 第 583、586、589、595 行，共 4 处单行微调）
...
- [x] F1. 编译验证：xelatex×3 + bibtex 无 error、无 undefined reference/citation 警告（由主会话直接执行）
- [x] F2. 文本通读：4 处修改只涉及 tikzpicture 内坐标/选项，不改变任何语义与图注，文件其余部分零改动
